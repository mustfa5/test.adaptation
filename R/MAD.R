#' This function calculates mean absolute difference (MAD) function as introduced in Wyse & McBride (2021)
#'
...
#'
#' @importFrom catR Ii
#' @return Returns a matrix for MAD in which every row is the MAD for each test taker
#' @export
#' @references
...
#'MAD(estimated.theta, items.administered, bank)

MAD <- function(estimated.theta, items.administered, bank) {
  require(catR)
  estimated.theta <- as.matrix(estimated.theta)
  MAD<- matrix(NA,nrow = nrow(estimated.theta))
  for (i in 1:nrow(estimated.theta)) {
    L <- length(items.administered[i,])
    dif <- bank[unlist(items.administered[i,]),2]
    pay<- sum(abs(dif-(as.vector(estimated.theta[i,]))))
    payda<- L
    MAD[i,] <- pay/payda
  }
  return(MAD)
