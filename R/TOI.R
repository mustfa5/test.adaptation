#' This function calculates theoretical optimal information (TOI ) function as introduced in Kingsbury & Wise (2020)
#'
...
#'
#' @importFrom catR Ii
#' @return Returns a single numeric value for TOI
#' @references
...
#'   diff[k,]=bank[xx,2]
#' }
#' TOI(theta, t.hat, items.administered, bank)


TOI <- function(theta,t.hat, items.administered, bank) {
  require(catR)
  ETI=matrix(ncol = 1,nrow = length(theta))
  ...
  inf.items[,2]=theta[i]
  ETI[i,1] =IAj/sum(Ii(t.hat[i],inf.items)$Ii)
  }
  TOI=mean(ETI)
  return(noquote(paste("TOI=",round(TOI,digits = 2))))
  }
