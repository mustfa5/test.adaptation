#' This function calculates mean difference (MD) function as introduced in Wyse & McBride (2021)
#'
...
#'
#' @importFrom catR Ii
#' @return Returns a matrix for MD in which every row is the MD for each test taker
#' @export
#' @references
...
#'MD(estimated.theta, items.administered, bank)

MD <- function(estimated.theta, items.administered, bank) {
  require(catR)
  estimated.theta <- as.matrix(estimated.theta)
  MD<- matrix(NA,nrow = nrow(estimated.theta))
  for (i in 1:nrow(estimated.theta)) {
    L <- length(items.administered[i,])
    dif <- bank[unlist(items.administered[i,]),2]
    pay<- sum(dif-(as.vector(estimated.theta[i,])))
    payda<- L
    MD[i,] <- pay/payda
  }
  return(MD)
